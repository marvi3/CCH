...
struct less_than_rank {
    inline bool operator() (const unsigned &x, const unsigned &y) {
        return (globalGraph->backward_order[x] < globalGraph->backward_order[y]);
    }
};
...
void contractor::contractNodes(CCHGraph *g) {
    ...
    g->backward_order = backwardOrder(g->order);
    ...
    for (unsigned k = 0; k < g->first_out.size() - 1; ++k) {
        ...
        for (unsigned i = 0; i < edges[x].size(); ++i) {
            ...
            if (g->backward_order[x] >= g->backward_order[y1]) {
                continue;
            }

            for (unsigned j = i; j < edges[x].size(); ++j) {
                cc++;
                unsigned y2 = edges[x][j];

                if (g->backward_order[x] >= g->backward_order[y2]) continue;
