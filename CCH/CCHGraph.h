...
namespace std {
	...
	class CCHGraph {
	public:
        CCHGraph(string directory, string order_name, bool temp_travel);
		...
		virtual ~CCHGraph();
				...
				unsigned getWeightUp(unsigned x, unsigned y);
				unsigned getWeightDown(unsigned x, unsigned y);
				void initReverseHead();

		vector<unsigned> first_out;
		vector<unsigned> head;
		vector<unsigned> weight;
		vector<unsigned> reverse_head;
		...
		vector<map<unsigned, unsigned>> weight_Position;
	...
	private:
                void initiallize(string directory, string order_name);
	};
