...
class QueryServletPrototype : public MapServer::Servlet {

public:

        QueryServletPrototype(std::string handler, Algorithm& algorithm, const std::vector<Geometry::Point>& coordinates, const COORDINATE_TREE_TYPE& cs,
                              vector<unsigned> first_out, vector<unsigned> head) :
		Servlet(handler),
    ...
    virtual void Process(mg_connection* conn, const mg_request_info* requestInfo) {
        ...
        if (targetNode == -1) {
            ...
            sendResponse(conn, algorithm.run2(sourceNode, targetNode, first_out, head));
        }
